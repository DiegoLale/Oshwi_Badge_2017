...
secodsWithMillis Clock::_currentTime;
...
bool Clock::updateTime()
{
  ...
  _currentTime.seconds = (secsSince1900 - seventyYears);
  _currentTime.millisec = packetBuffer[44]*1000/256;
...
secodsWithMillis Clock::getTime()
{
  unsigned long milliseconds = millis();
  unsigned long elapsed = milliseconds - _lastUpdate;
  _currentTime.millisec += elapsed;
  _lastUpdate = milliseconds;

  if (_currentTime.millisec > 1000)
  {
    _currentTime.seconds += _currentTime.millisec / 1000;
    _currentTime.millisec %= 1000;
  }
